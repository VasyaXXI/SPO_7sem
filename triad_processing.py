...
class Triad:
    ...
    def triad_op(self, fl=True):
        ...
        print('PN after triad optimization:')
        self.triads_to_pm()
    ...
    def add_triad(self, t, fl = True):
        ...
        if t[3].get_type() == 'CONST':
            ...
            if True:#not self.found_triad_link(t[1].get_value()):
                if t[1].get_type() != 'TR':
                    out.append(t[1])
                else:
        ...
        while count < len(self.triads):
            ...
            if self.triads[count][3].get_type() == 'ASSIGN_OP':
                if not variability:
                    ...
                    #self.triads[count][1].set_type = '_VAR'
                    #ind = self.find_value(self.triads[count][1].get_value(), False)
                    #if ind is not None:
                        #del self.value_table[ind]
                    count += 1
                    continue
        ...
        while count < len(self.triads):
            if self.triads[count][3].get_type() == 'CONST':
                if not self.found_triad_link(self.triads[count], count):
                    ...
                    count -= 1
            # если присваивание диджит, до этого тож, и между ними переменной нет, то дел
            #если пр диджит и после этого не упоминается - делит
...
def print_triads(triads):
    ...
    for t in triads:
        ...
        # print('''({}: '{}')'''.format(t[3].get_type(), t[3].get_value()), end=' ')
    print()
